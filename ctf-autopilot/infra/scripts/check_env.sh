...
#   curl -fsSL https://raw.githubusercontent.com/huynhthanhthao-uel/ctf-compass/main/ctf-autopilot/infra/scripts/check_env.sh | bash
#
#   # Check only (no install)
#   curl -fsSL https://raw.githubusercontent.com/huynhthanhthao-uel/ctf-compass/main/ctf-autopilot/infra/scripts/check_env.sh | bash -s -- --check-only
#
#   # Auto-install missing dependencies
#   curl -fsSL https://raw.githubusercontent.com/huynhthanhthao-uel/ctf-compass/main/ctf-autopilot/infra/scripts/check_env.sh | sudo bash -s -- --install
#
# OPTIONS:
...
for arg in "$@"; do
    case $arg in
        ...
        --help|-h)
            ...
            echo "  curl -fsSL https://raw.githubusercontent.com/huynhthanhthao-uel/ctf-compass/main/ctf-autopilot/infra/scripts/check_env.sh | bash"
            exit 0
            ;;
    esac
done
...
main() {
    if [[ "$JSON_MODE" != "true" ]]; then
        ...
        echo -e "${CYAN}║            github.com/huynhthanhthao-uel/ctf-compass               ║${NC}"
        echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
        echo ""
    fi
    ...
    if [[ "$JSON_MODE" == "true" ]]; then
        ...
        if [[ $errors -eq 0 && $warnings -eq 0 ]]; then
            ...
            echo -e "  ${CYAN}curl -fsSL https://raw.githubusercontent.com/huynhthanhthao-uel/ctf-compass/main/ctf-autopilot/infra/scripts/install_ubuntu_24.04.sh | sudo bash${NC}"
            ...
            if [[ "$CHECK_ONLY" != "true" ]]; then
                ...
                echo -e "  ${CYAN}curl -fsSL https://raw.githubusercontent.com/huynhthanhthao-uel/ctf-compass/main/ctf-autopilot/infra/scripts/check_env.sh | sudo bash -s -- --install${NC}"
            fi
        fi
        echo ""
    fi
